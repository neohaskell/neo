...
- 2026-04-23: Fixed `neo build` failure in Git repositories where `.cabal` files were untracked. Updated `neo new` to generate and commit the `.cabal` file during initialization. Added a warning comment to the `.cabal` template and safeguarded `.gitignore` against ignoring cabal files. Verified with successful builds of new projects.
- 2026-07-17: Implemented and independently reviewed the first Neo IDE collaboration vertical slice: `neo ide --share/--join`, signed bearer-capability messages over native Iroh gossip, host-authoritative sequencing and transactional atomic persistence, explicit rejection/repair, late-browser and sequence-gap snapshot repair, move-only React Flow synchronization in flat and feature views, peer presence/multicursors, and collaboration-aware UI restrictions. Verified with two independent Neo processes and browser contexts in both move directions, feature-mode move persistence, browser/process reconnect, malformed-ticket handling, 12 collaboration tests, all 635 Rust unit tests, all 384 frontend tests, TypeScript/build checks, and the non-blocked integration suites. The full `cargo test` run remains blocked by three pre-existing long-running Nix/CLI integration tests documented in `NEXT_STEP.md`.
